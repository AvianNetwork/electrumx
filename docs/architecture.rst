...
Not started until the Block Processor has caught up with aviand.

Daemon
------

Encapsulates the RPC wire protocol with aviand for the whole server.
Transparently handles temporary aviand connection errors, and fails
over if necessary.
...
from aviand.  Once it has caught up it additionally asks the Daemon
to refresh its view of aviand's mempool transaction hashes.  Serves
blocks to the Block Processor via a queue.

Mempool
-------

Handles all the details of maintaining a representation of aviand's
